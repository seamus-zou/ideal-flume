...
public class YeeFileEventReader3 {
    private final static Logger logger = LoggerFactory.getLogger(YeeFileEventReader3.class);
    ...
    private YeeFileEventReader3(ClientProps clientProps, CacheClient cacheClient,
            ...
            int sendThreadCount, DataFileType dataFileType, String hostname, String baseDir,
            String currentDayDir, int currentHourDir, boolean needSwitchHour, String dynamicDirReg,
            boolean isRepair, String busDir
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Initializing {} with directory={}, deserializer={}", new Object[] {
                    YeeFileEventReader3.class.getSimpleName(), spoolDirectory, deserializerType});
        }
        ...
        if (needSwitchHour) {
            ...
            while (this.currentHourDir < 23) {
                logger.info("check path: {}", spoolDirectory);
        ...
        synchronized (unreadFilesCounterMap) {
            if (!unreadFilesCounterMap.containsKey(remoteUrl)) {
                unreadFilesCounterMap.put(remoteUrl,
                        Counters.create("reader", busDir + "-unreadFiles", 1));
            }
        }
        synchronized (unreadFileSizeCounterMap) {
            if (!unreadFileSizeCounterMap.containsKey(remoteUrl)) {
                unreadFileSizeCounterMap.put(remoteUrl,
                        Counters.create("reader", busDir + "-unreadFileSize", 1));
            }
        }
        ...
        for (CollectFile file : checkFiles) {
            ...
            try {
                reader = new BufferedReader(new InputStreamReader(
                        listClient.retrieveFileStream(spoolDirectory + file.getName())));
    ...
    public boolean fileend() throws IOException {
        if (doend.get()) {
            ...
            return (System.currentTimeMillis() - doendTmp > 60000);
        } else {
    ...
    public void setSpoolDirectory(String dayDir) {
        ...
        this.spoolDirectory = baseDir + dayDir + (needSwitchHour ? "/00/" : "/") + busDir;
    }
    ...
    public void switchNextHour() throws Exception {
        ...
        while (tmpHour < 23) {
            tmpHour++;
            String path =
                    baseDir + currentDayDir + "/" + (tmpHour < 10 ? "0" + tmpHour : tmpHour) + "/" + busDir;
    ...
    public static class Builder {
        ...
        public Builder busDir(String busDir) {
            this.busDir = busDir;
            return this;
        }

        public YeeFileEventReader3 build() throws Exception {
            return new YeeFileEventReader3(clientProps, cacheClient, spoolDirectory, completedSuffix,
                    ...
                    baseDir, currentDayDir, currentHourDir, needSwitchHour, dynamicDirReg, isRepair,
                    busDir);
        }
    }
        ...
        if (!committed) {
            ...
            if (!currentFile.isPresent()) {
                ...
                if (currentFile.isPresent()) {
                    // 读取过文件但没完成，跳过已读的行
                    int redisReadLine =
                            getReadLine(spoolDirectory + currentFile.get().getFile().getName());
                    if (redisReadLine > 0) {
                        eventsReads = redisReadLine;
                        EventDeserializer des = currentFile.get().getDeserializer();
                        des.readEvents(redisReadLine);
        ...
        if (org.apache.commons.lang3.StringUtils.isNotEmpty(currentDayDir)) {
            if (needSwitchHour) {
                headers.put("timestamp", DateTimeUtils
                        .parseDate(currentDayDir + " " + currentHourDir, dynamicDirReg + " HH")
                        .getTime() + "");
            } else {
