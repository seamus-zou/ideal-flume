...
public class YeeFileEventReader {
    private final static Logger logger = LoggerFactory.getLogger(YeeFileEventReader.class);
    ...
    /** 缓存时是否使用绝对路径 */
    private final boolean cacheAbsolutePath;
    ...
    private YeeFileEventReader(ClientProps clientProps, CacheClient cacheClient,
            ...
            final long listInterval, int listDays, boolean cacheAbsolutePath) throws Exception {
        // Sanity checks
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Initializing {} with directory={}, deserializer={}", new Object[] {
                    YeeFileEventReader.class.getSimpleName(), spoolDirectory, deserializerType});
        }
        ...
        this.cacheAbsolutePath = cacheAbsolutePath;
...
//                            	 logger.info("===遍历异常了");
    ...
    private void listFiles() {
        ...
        logger.info("===remoteFiles："+remoteFiles.size());
...
//                logger.info("===补数据，停止");
                ...
                CollectFileFilter filter = new CollectFileFilter() {
                    public boolean accept(CollectFile candidate) {
                        ...
                        if (candidate.isDirectory() || fileName.endsWith(completedSuffix)
                                ...
                                || !fileName.startsWith(prefix) || isReaded(candidate)
                                || isReading(candidate) || isFailed(candidate) ||
                        // 修改1分钟之外才会被处理
    ...
    private boolean isReaded(CollectFile file) {
        String fileName = cacheAbsolutePath ? file.getAbsolutePath() : file.getName();
        return !isRepair && cacheClient.isReaded(fileName);
    }

    private boolean isReading(CollectFile file) {
        String fileName = cacheAbsolutePath ? file.getAbsolutePath() : file.getName();
        return !isRepair && cacheClient.isReading(fileName);
    }

    private boolean isFailed(CollectFile file) {
        String fileName = cacheAbsolutePath ? file.getAbsolutePath() : file.getName();
        return !isRepair && cacheClient.isFailed(fileName);
    }

    private void cacheReaded(CollectFile file) throws Exception {
        String fileName = cacheAbsolutePath ? file.getAbsolutePath() : file.getName();
        cacheClient.cacheReaded(fileName);
    }

    private boolean cacheReading(CollectFile file) throws Exception {
        String fileName = cacheAbsolutePath ? file.getAbsolutePath() : file.getName();
        return cacheClient.cacheReading(fileName);
    }

    private void cacheReadedLine(CollectFile file, int lines) {
        String fileName = cacheAbsolutePath ? file.getAbsolutePath() : file.getName();
        cacheClient.cacheReadedLines(fileName, lines);
    }

    private int getReadLine(CollectFile file) {
        String fileName = cacheAbsolutePath ? file.getAbsolutePath() : file.getName();
        return cacheClient.getFileReadedLines(fileName);
    }

    private void removeReading(CollectFile file) {
        String fileName = cacheAbsolutePath ? file.getAbsolutePath() : file.getName();
        cacheClient.removeReading(fileName);
    }

    private void cacheFailFile(CollectFile file) throws Exception {
        String fileName = cacheAbsolutePath ? file.getAbsolutePath() : file.getName();
    ...
    public static class Builder {
        ...
        public Builder cacheAbsolutePath(boolean cacheAbsolutePath) {
            this.cacheAbsolutePath = cacheAbsolutePath;
            return this;
        }

        public YeeFileEventReader build() throws Exception {
            return new YeeFileEventReader(clientProps, cacheClient, spoolDirectory, completedSuffix,
                    ...
                    isRepair, listInterval, listDays, cacheAbsolutePath);
        }
    }
    ...
    public List<Event> readEvents(int numEvents) throws Exception {
        if (!committed) {
            ...
            // 读取过文件但没完成，跳过已读的行
            int redisReadLine = getReadLine(currentFile.get().getFile());
            if (redisReadLine > 0) {
    ...
    public void commit() throws Exception {
        if (!committed && currentFile.isPresent()) {
            ...
            cacheReadedLine(currentFile.get().getFile(), eventsReads);
        }
    }
    ...
    private Optional<FileInfo> getNextFile() throws Exception {
        ...
        while (isReaded(f) || isReading(f) || isFailed(f)) {
