...
public class YeeFileEventReader2 {
  private final static Logger logger = LoggerFactory.getLogger(YeeFileEventReader2.class);
  ...
  private YeeFileEventReader2(ClientProps clientProps, CacheClient cacheClient,
    ...
    if (logger.isDebugEnabled()) {
      logger.debug("Initializing {} with directory={}, deserializer={}", new Object[] {
          YeeFileEventReader2.class.getSimpleName(), spoolDirectory, deserializerType});
    }
  ...
  public static class Builder {
    ...
    public YeeFileEventReader2 build() throws Exception {
      return new YeeFileEventReader2(clientProps, cacheClient, spoolDirectory, completedSuffix,
          deserializerType, deserializerContext, deletePolicy, inputCharset, consumeOrder, bufSize,
          prefix, suffix, channelCount, sendThreadCount, dataFileType, hostname, baseDir,
  ...
  private Optional<FileInfo> openFile(CollectFile file) {
    ...
    try {
      ...
      if (DataFileType.GZ == dataFileType) {
        fileIn = new GZIPInputStream(readClient.retrieveFileStream(spoolDirectory + file.getName()),
            bufSize);
      } else {
