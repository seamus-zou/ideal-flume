...
public class PollingZooKeeperConfigurationProvider extends AbstractZooKeeperConfigurationProvider
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(PollingZooKeeperConfigurationProvider.class);
  ...
  public PollingZooKeeperConfigurationProvider(String agentName, String zkConnString,
      String basePath, EventBus eventBus) {
  ...
  public void start() {
    ...
    try {
      client.start();
      try {
        agentNodeCache = new NodeCache(client, "/" + getAgentName() + "/conf");
  ...
  @Override
  public void stop() {
    LOGGER.debug("Stopping zk provider ...");
    ...
    try {
      LOGGER.debug("Stopping zk client ...");
      client.close();
    } catch (Exception e) {
