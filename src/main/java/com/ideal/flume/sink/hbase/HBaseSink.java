 ...
 * the License.
 */
package com.ideal.flume.sink.hbase;
...
/**
 *
 * A simple sink which reads events from a channel and writes them to HBase. The Hbase configuration
 * is picked up from the first <tt>hbase-site.xml</tt> encountered in the classpath. This sink
 * supports batch reading of events from the channel, and writing them to Hbase, to minimize the
 * number of flushes on the hbase tables. To use this sink, it has to be configured with certain
 ...
 * <p>
 * This sink will commit each transaction if the table's write buffer size is reached or if the
 * number of events in the current transaction reaches the batch size, whichever comes first.
 * <p>
 * Other optional parameters are:
 * <p>
 * <tt>serializer:</tt> A class implementing {@link HbaseEventSerializer}. An instance of this class
 * will be used to write out events to hbase.
 * <p>
 * <tt>serializer.*:</tt> Passed in the configure() method to serializer as an object of
 * {@link org.apache.flume.Context}.
 * <p>
 * <tt>batchSize: </tt>This is the batch size used by the client. This is the maximum number of
 * events the sink will commit per transaction. The default batch size is 100 events.
 * <p>
 *
 * <strong>Note: </strong> While this sink flushes all events in a transaction to HBase in one shot,
 * Hbase does not guarantee atomic commits on multiple rows. So if a subset of events in a batch are
 * written to disk by Hbase and Hbase fails, the flume transaction is rolled back, causing flume to
 * write all the events in the transaction all over again, which will cause duplicates. The
 * serializer is expected to take care of the handling of duplicates etc. HBase also does not
 * support batch increments, so if multiple increments are returned by the serializer, then HBase
 * failure will cause them to be re-written, when HBase comes back up.
 */
public class HBaseSink extends AbstractSink implements Configurable {
  ...
  public void start() {
    ...
    try {
      ...
      throw new FlumeException(
          "Error getting column family from HBase." + "Please verify that the table " + tableName
              + " and Column Family, " + Bytes.toString(columnFamily) + " exists in HBase, and the"
              + " current user has permissions to access that table.",
          e);
    }

    statCounter = StatCounters.create(StatType.SINK, tableName);
  ...
  public void configure(Context context) {
    ...
    if (eventSerializerType == null || eventSerializerType.isEmpty()) {
      eventSerializerType = "com.ideal.flume.sink.hbase.RegexHbaseEventSerializer";
      logger.info("No serializer defined, Will use default");
    }
    serializerContext
        .putAll(context.getSubProperties(HBaseSinkConfigurationConstants.CONFIG_SERIALIZER_PREFIX));
    columnFamily = cf.getBytes(Charsets.UTF_8);
    try {
      Class<? extends HbaseEventSerializer> clazz =
          (Class<? extends HbaseEventSerializer>) Class.forName(eventSerializerType);
    ...
    Integer port = null;
    /**
     * HBase allows multiple nodes in the quorum, but all need to use the same client port. So get
     * the nodes in host:port format, and ignore the ports for all nodes except the first one. If no
     * port is specified, use default.
     */
    if (zkQuorum != null && !zkQuorum.isEmpty()) {
      ...
      for (int i = 0; i < length; i++) {
        ...
        if (port == null) {
          ...
          throw new FlumeException(
              "All Zookeeper nodes in the quorum must " + "use the same client port.");
        }
      }
      if (port == null) {
  ...
  /**
   * The method getFamilyMap() is no longer available in Hbase 0.96. We must use reflection to
   * determine which version we may use.
   */
  @VisibleForTesting
  ...
  /**
   * Perform "compression" on the given set of increments so that Flume sends the minimum possible
   * number of RPC operations to HBase per batch.
   * 
  ...
  private List<Increment> coalesceIncrements(Iterable<Increment> incs) {
    ...
    for (Map.Entry<byte[], Map<byte[], NavigableMap<byte[], Long>>> rowEntry : counters
        .entrySet()) {
  ...
  /**
   * Helper function for {@link #coalesceIncrements} to increment a counter value in the passed data
   * structure.
   * 
   ...
   */
  private void incrementCounter(Map<byte[], Map<byte[], NavigableMap<byte[], Long>>> counters,
