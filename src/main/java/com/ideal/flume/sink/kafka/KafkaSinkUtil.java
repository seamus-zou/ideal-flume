...
public class KafkaSinkUtil {
	...
	/**
	 * Some of the producer properties are especially important We documented
	 * them and gave them a camel-case name to match Flume config If user set
	 * these, we will override any existing parameters with these settings.
	 * Knowledge of which properties are documented is maintained here for now.
	 * If this will become a maintenance issue we'll set a proper data
	 * structure.
	 */
	private static void addDocumentedKafkaProps(Context context, Properties kafkaProps) throws ConfigurationException {
		...
		String compressionCodecKey = context.getString(KafkaSinkConstants.COMPRESSION_CODEC_FLUME_KEY);
		if (compressionCodecKey != null) {
			kafkaProps.put(KafkaSinkConstants.COMPRESSION_CODEC_KEY, compressionCodecKey);
		}
	}
	...
	/**
	 * Add all configuration parameters starting with "kafka" to producer
	 * properties
	 */
