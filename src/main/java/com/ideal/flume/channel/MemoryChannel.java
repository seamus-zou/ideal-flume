 ...
 * the License.
 */
package com.ideal.flume.channel;
...
/**
 * <p>
 * MemoryChannel is the recommended channel to use when speeds which writing to disk is impractical
 * is required or durability of data is not required.
 * </p>
 * <p>
...
public class MemoryChannel extends BasicChannelSemantics {
  ...
  private static final Integer defaultCapacity = 100000;
  private static final Integer defaultTransCapacity = 100000;
  ...
  private static final Integer defaultKeepAlive = 30;
  ...
  private class MemoryTransaction extends BasicTransactionSemantics {
    ...
    protected Event doTake() throws InterruptedException {
      ...
      Preconditions.checkNotNull(event,
          "Queue.poll returned NULL despite semaphore " + "signalling existence of entry");
      takeList.put(event);
    ...
    protected void doCommit() throws InterruptedException {
      ...
      if (remainingChange < 0) {
        if (!bytesRemaining.tryAcquire(putByteCounter, keepAlive, TimeUnit.SECONDS)) {
          throw new ChannelException("Cannot commit transaction. Heap space " + "limit of "
              + byteCapacity + "reached. Please increase heap space"
  ...
  // this allows local threads waiting for space in the queue to commit
  // without denying access to the
   ...
   * <li>capacity = type long that defines the total number of events allowed at one time in the
   * queue.
   * <li>transactionCapacity = type long that defines the total number of events allowed in one
   * transaction.
  ...
  public void configure(Context context) {
    try {
      ...
      LOGGER.warn("Invalid capacity specified, initializing channel to " + "default capacity of {}",
          defaultCapacity);
    }
    ...
    try {
      transCapacity = context.getInteger("transactionCapacity", capacity);
    } catch (NumberFormatException e) {
      transCapacity = capacity;
      LOGGER.warn("Invalid transation capacity specified, initializing channel"
          + " to default capacity of {}", capacity);
    }
    ...
    counter = Counters.create("memory-channel-" + getName(), "event", 1);
  }
