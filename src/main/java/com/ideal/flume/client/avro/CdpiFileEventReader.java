...
public class CdpiFileEventReader {
    private final static Logger logger = LoggerFactory.getLogger(CdpiFileEventReader.class);
    ...
    private CdpiFileEventReader(ClientProps clientProps, CacheClient cacheClient,
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Initializing {} with directory={}, deserializer={}", new Object[] {
                    CdpiFileEventReader.class.getSimpleName(), spoolDirectory, deserializerType});
        }
    ...
    private Set<String> listCheckFiles(String path) throws Exception {
        // 如果有check文件，获取所有的check文件
        List<CollectFile> checkFiles =
                listClient.listFiles(path, new CollectFileFilter() {
                    @Override
                    public boolean accept(CollectFile file) {
                        return file.getName().endsWith(".chk");
                    }
                });
        Set<String> finishedFileNames = Sets.newHashSet();
            ...
            for (CollectFile file : checkFiles) {
                String dataFileName =
                        file.getAbsolutePath().substring(0, file.getAbsolutePath().length() - 4)
                                + ".cdr.gz";
                if (isReaded(dataFileName) || isFailed(dataFileName) || isReading(dataFileName)) {
                    continue;
                }
                finishedFileNames
                        .add(file.getName().substring(0, file.getName().length() - 4) + ".cdr.gz");
            }
        }
        return finishedFileNames;
    }
        ...
        if (remoteFiles.size() == 0) {
            ...
            try {
                ...
                CollectFileFilter filter = new CollectFileFilter() {
                    public boolean accept(CollectFile candidate) {
                        ...
                        if (candidate.isDirectory() || fileName.endsWith(completedSuffix)
                                ...
                                || !fileName.startsWith(prefix) || (System.currentTimeMillis()
                                        - candidate.getTimeInMillis() < 60000)) {
                            if (logger.isDebugEnabled()) {
                ...
                if (dynamicYear || dynamicMonth || dynamicDay || dynamicHour) {
                    for (int i = listDays; i <= 0; i++) {
                        ...
                        try {
                            ...
                            if (dynamicHour) {
                                ...
                                Set<String> finishedFileNames = listCheckFiles(path);
                                List<CollectFile> fs = listClient.listFiles(path, filter);
                                for (CollectFile f : fs) {
                                    if (finishedFileNames.contains(f.getName())) {
                                        files.add(f);
                                    }
                                }
                            }
                    ...
                    Set<String> finishedFileNames = listCheckFiles(spoolDirectory);
                    List<CollectFile> fs = listClient.listFiles(spoolDirectory, filter);
                    for (CollectFile f : fs) {
                        if (finishedFileNames.contains(f.getName())) {
                            files.add(f);
                        }
    ...
    public static class Builder {
        ...
        public CdpiFileEventReader build() throws Exception {
            return new CdpiFileEventReader(clientProps, cacheClient, spoolDirectory,
                    completedSuffix, deserializerType, deserializerContext, deletePolicy,
                    inputCharset, consumeOrder, bufSize, prefix, suffix, channelCount,
                    sendThreadCount, dataFileType, hostname, isRepair, listInterval, listDays);
        }
    }
