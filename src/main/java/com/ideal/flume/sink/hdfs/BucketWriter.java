...
import com.ideal.flume.sink.hdfs.HDFSEventSink.WriterCallback;

/**
 * Internal API intended for HDFSSink use. This class does file rolling and handles file formats and
 * serialization. Only the public methods in this class are thread safe.
 */
class BucketWriter {
  ...
  BucketWriter(long rollInterval, long rollSize, long rollCount, long batchSize, Context context,
      String filePath, String fileName, String inUsePrefix, String inUseSuffix, String fileSuffix,
      CompressionCodec codeC, CompressionType compType, HDFSWriter writer,
      ScheduledExecutorService timedRollerPool, UserGroupInformation user, int idleTimeout,
      WriterCallback onCloseCallback, String onCloseCallbackPath, long callTimeout,
      ExecutorService callTimeoutPool, long retryInterval, int maxCloseTries) {
  ...
  private Method getRefIsClosed() {
    try {
      ...
      LOG.warn(
          "isFileClosed is not available in the " + "version of HDFS being used. Flume will not "
              + "attempt to close files if the close fails on " + "the first attempt",
          e);
      return null;
    }
  }
  ...
  /**
   * Close the file handle and rename the temp file to the permanent filename. Safe to call multiple
   * times. Logs HDFSWriter.close() exceptions. This method will not cause the bucket writer to be
   * dereferenced from the HDFS sink that owns it. This method should be used only when size or
   * count based rolling closes this file.
   * 
  ...
  private Callable<Void> createScheduledCloseCallable(final CallRunner<Void> closeCallRunner) {
    ...
    return new Callable<Void>() {
      ...
      public Void call() throws Exception {
        if (closeTries >= maxCloseTries) {
          LOG.warn("Unsuccessfully attempted to close " + path + " " + maxCloseTries
              + " times. File may be open, " + "or may not have been renamed.");
          return null;
        }
        closeTries++;
        try {
          ...
          LOG.warn("Closing file: " + path + " failed. Will " + "retry again in " + retryInterval
              + " seconds.", e);
  ...
  public synchronized void close(boolean callCloseCallback)
    ...
    if (isOpen) {
      try {
        ...
        LOG.warn("failed to close() HDFSWriter for file (" + bucketPath + "). Exception follows.",
            e);
        failedToClose = true;
        final Callable<Void> scheduledClose = createScheduledCloseCallable(closeCallRunner);
        timedRollerPool.schedule(scheduledClose, retryInterval, TimeUnit.SECONDS);
      }
      isOpen = false;
    } else {
  ...
  /**
   * Open file handles, write data, update stats, handle file rolling and batching / flushing.
   * <br />
   * If the write fails, the file is implicitly closed and then the IOException is rethrown. <br />
   * We rotate before append, and not after, so that the active file rolling mechanism will never
   * roll an empty file. This also ensures that the file creation time reflects when the first event
   * was written.
   *
   * @throws IOException
   * @throws InterruptedException
  ...
  public synchronized void append(final Event event) throws IOException, InterruptedException {
    ...
    try {
      ...
      try {
        ...
        LOG.warn("Caught IOException while closing file (" + bucketPath + "). Exception follows.",
            e2);
      }
      throw e;
    }
  ...
  public void addEventCounter(long size) {
    eventCounter += size;
  }
  ...
  /**
   * Execute the callable on a separate thread and wait for the completion for the specified amount
   * of time in milliseconds. In case of timeout cancel the callable and throw an IOException
   */
  private <T> T callWithTimeout(final CallRunner<T> callRunner)
    ...
    try {
      ...
      throw new IOException(
          "Callable timed out after " + callTimeout + " ms" + " on file: " + bucketPath, eT);
  ...
  /**
   * Simple interface whose <tt>call</tt> method is called by {#callWithTimeout} in a new thread
   * inside a {@linkplain java.security.PrivilegedExceptionAction#run()} call.
   * 
   * @param <T>
   */
