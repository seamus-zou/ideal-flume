...
public class YeeFileEventReader6 {
    private final static Logger logger = LoggerFactory.getLogger(YeeFileEventReader6.class);
    ...
    private YeeFileEventReader6(ClientProps clientProps, CacheClient cacheClient,
            ...
            final long listInterval) throws Exception {
        // Sanity checks
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Initializing {} with directory={}, deserializer={}", new Object[] {
                    YeeFileEventReader6.class.getSimpleName(), spoolDirectory, deserializerType});
        }
        ...
        this.remoteUrl = StringUtils.trimToEmpty(clientProps.getIp())
                + StringUtils.trimToEmpty(clientProps.getWorkingDirectory()) + prefix;
    ...
    private void listFiles() {
        ...
        if (remoteFiles.size() == 0) {
            ...
            try {
                ...
                final Set<String> finishedFileNames = Sets.newHashSet();
                if (CollectionUtils.isNotEmpty(checkFiles)) {
                    for (CollectFile file : checkFiles) {
                        String dataFileName = file.getAbsolutePath().substring(0,
                                file.getAbsolutePath().length() - 4) + ".txt";
                        if (isReaded(dataFileName) || isFailed(dataFileName)
                                || isReading(dataFileName)) {
                            continue;
                        }
                        finishedFileNames.add(
                ...
                CollectFileFilter filter = new CollectFileFilter() {
                    public boolean accept(CollectFile candidate) {
                        ...
                        if (candidate.isDirectory() || fileName.endsWith(completedSuffix)
                                ...
                                || !finishedFileNames.contains(fileName)) {
                            if (logger.isDebugEnabled()) {
                ...
                logger.info("list remote files done. Files: "
                        + unreadFilesCounterMap.get(remoteUrl).getValue());
            } catch (Throwable e) {
                logger.error("", e);
            }
        }
    ...
    public static class Builder {
        ...
        public YeeFileEventReader6 build() throws Exception {
            return new YeeFileEventReader6(clientProps, cacheClient, spoolDirectory, completedSuffix,
    ...
    public List<Event> readEvents(int numEvents) throws Exception {
        ...
        headers.put("%{path}", fileName.substring(fileName.length() - 15, fileName.length() - 7));
        headers.put("%{fileName}", currentFile.get().getFile().getName());
